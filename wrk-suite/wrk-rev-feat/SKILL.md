...
3. **Answer these four questions, each with evidence (file:line, diff hunks, quoted guideline):**
   ...
   **A4. Approach (feature design gate)**
   Features are design decisions, not just diffs — so evaluate *how* this was
   built, not only whether it works.

   1. **Explain the approach taken.** In a few lines, describe the implementation
      approach actually used in the diff (the core design: where the logic lives,
      the data flow, the extension point/pattern it follows, the key trade-off it
      commits to). Ground it in real file:line / hunks — not a restatement of the
      issue.
   2. **What approaches did you consider?** List the credible **alternatives** — at
      least the one the PR chose plus **one or more** other real options (e.g. a
      different extension point, reusing an existing helper vs. new abstraction, a
      library vs. hand-rolled, sync vs. async, config-driven vs. hard-coded). For
      each, one line on how it would have solved the same requirement. If the issue
      or repo conventions effectively force a single approach, say so and cite why.
   3. **Comparison table (required).** Compare the approaches on the criteria that
      matter for this feature. Mark the chosen one, and make the row values
      concrete, not generic praise:

      | Approach | Fit to issue | Scope / diff size | Reuse of existing code | Maintainability | Risk / edge cases | Chosen |
      |----------|--------------|-------------------|------------------------|-----------------|-------------------|--------|
      | A (…)    | …            | …                 | …                      | …               | …                 | ✅/❌  |
      | B (…)    | …            | …                 | …                      | …               | …                 | ✅/❌  |

      (Add/rename criteria columns to fit the feature; keep the **Chosen** column.)

   Then judge: is the chosen approach the right one, or does a considered
   alternative clearly beat it on the criteria above? If the diff commits to a
   weaker approach than a viable alternative (more scope, worse reuse, higher risk
   for no offsetting benefit) → call it out as a finding (contributes to
   **NEEDS WORK** when it materially harms the contribution). If the chosen approach
   is sound, say so with the table as evidence.
